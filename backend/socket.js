...
module.exports = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("setup", async (id) => {
      try {
        userRoomMap[id] = socket.id;
        socket.join(id);
        console.log("User joined room", id);
        socket.emit("user setup", id);

        // Find all conversations of user and send other user online event
        const conv = await Conversation.find({ members: { $in: [id] } });
    ...
    socket.on("join-chat", async (data) => {
      try {
        ...
        // Find if other user is online
        const otheruserSocket = io.sockets.adapter.rooms.get(otheruser.toString());
        console.log("User is:", data.user);
        io.to(data.room).emit("user-joined-room", data.user);
    ...
    socket.on("send-message", async (data) => {
      try {
        ...
        const conversation = await Conversation.findById(data.data.conversationId)
          .populate("members", "-password");
        conversation.latestmessage = data.data.text;
        let isbot = false;
        ...
        // Update seen status for members in the room
        members.forEach((member) => {
          if (member !== data.data.sender) {
            const memberroom = io.sockets.adapter.rooms.get(member);
            if (memberroom && memberroom.has(socket.id)) {
              data.data.seenby = [member];
            }
          }
        });

        // Emit received message to all clients in the room
        io.to(data.data.conversationId).emit("receive-message", data);

        // Check if any member is a bot and handle bot response
        const botMember = conversation.members.find(member => member.email.includes("bot"));
        if (botMember) {
          isbot = true;
          io.to(data.data.conversationId).emit("typing", data); // Bot typing indicator
          
          const airesponse = await generateairesponse(data.data); // Generate AI response
          const aidata = {
            data: {
              conversationId: data.data.conversationId,
              sender: botMember,
          ...
          // Emit AI response and stop typing
          io.to(data.data.conversationId).emit("receive-message", aidata);
          io.to(data.data.conversationId).emit("stop-typing", data);
          await conversation.save();
        }
        ...
        if (!isbot) {
          // Update unread message counts for all members except the sender
          const ids = conversation.members.map((member) => member.id.toString());
          conversation.unread = conversation.unread.map((unread, index) => {
            if (index !== ids.indexOf(data.data.sender)) {
              return unread + 1;
            }
            return unread;
          });
    ...
    socket.on("disconnect", () => {
      console.log("A user disconnected");
      Object.keys(userRoomMap).forEach((userId) => {
        if (userRoomMap[userId] === socket.id) {
